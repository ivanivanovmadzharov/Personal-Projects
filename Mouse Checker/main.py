...
def on_left_click(event):
    mouse_click_label.config(text="Left Mouse Button Clicked")


def on_right_click(event):
    mouse_click_label.config(text="Right Mouse Button Clicked")


def main():
    root = tk.Tk()
    root.title("Mouse Checker")
    root.geometry("400x300")
    global mouse_position_label
    mouse_position_label = tk.Label(root, text="Mouse Position: ", font=("Helvetica", 16))
